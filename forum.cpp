...
forum::forum(const char* name)      //Forum constructor
{
...
forum::~forum()                     //Forum destructor
{
    int i, j;
    cout << "Forum with title: "<<  get_title() << " is about to be destroyed!"<< endl;
    for(i = 0;i < SIZE; i++)
    {
        cout << "Thread with title: "<<  get_thread_subject(i) << " is about to be destroyed!"<< endl;
        for(j = 0;j < SIZE; j++)
...
void forum::set_forum()
{
    ...
    for(i = 0; i < SIZE; i++ )
    {
        ...
        cout << "Thread with title: "<<  get_thread_subject(i) << " has just been created!"<< endl;  
    }  
}
...
void thread::set_thread(const string name,const string title, int j)
{
   ...
   //Array with post creators
    string temp2[][SIZE] = { {"author 1","author 2","author 3","author 4"},
    ...
    for(i = 0; i < SIZE; i++ )
    {
        sprintf(temp, "%s %d", title.c_str(), i);                       //Copy thread title
        if (i == 0) set_post(thread_creator, temp, temp1[j][i],j,i);    //First post creator == thread creator
        else set_post(temp2[j][i], temp, temp1[j][i],j,i);              //Else random post creator
    }     
}
...
string forum::get_thread_subject(int i)
{
    string subject;
    subject = threads[i].get_thread_subject();
    return subject;
}

string thread::get_thread_subject()
{
    return subject;
}
...
void forum::print()         //Function that prints whole forum
{
    int i, j, month, day, year;
    for(i = 0; i < SIZE; i++ )
    {
        cout << "\nThread title "<<  get_thread_subject(i) << endl;
        cout << "Thread writer: "<<  get_thread_creator(i) <<  endl; 
        get(month, day, year, i);
        cout << "Thread date: "<<  day << " "<< month << "  " << year <<  endl;
        for(j = 0; j < SIZE; j++ )
        {
            cout << "Post title: "<<  threads[i].get_post(j) <<  endl;
            cout << "Post id: "<<  threads[i].get_post_id(j) <<  endl;
            cout << "Post writer: "<<  threads[i].get_post_creator(j) <<  endl;
            threads[i].get(month, day, year);
            cout << "Post date: " <<  day << " "<< month << "  " << year <<  endl;
            cout << "Post text: "<<  threads[i].get_post_txt(j) <<  endl;
        }
    }
}

void forum::search(string temp)        //Function that searches certain thread
{
    int i, j, month, day, year;
    for(i = 0; i < SIZE; i++ )
    {
        if(temp == get_thread_subject(i))
        {
            cout << "\nThread title "<<  get_thread_subject(i) << endl;
            cout << "Thread writer: "<<  get_thread_creator(i) <<  endl; 
            get(month, day, year, i);
